...
public class AndFixManager {
	...
	public synchronized void removeOptFile(File file) {
		...
		if (optfile.exists() && !optfile.delete()) {
			Util_LOGFile.getInstance().write(TAG + "\n" + optfile.getName() + " delete error.");
			Log.e(TAG, optfile.getAbsolutePath() + " delete error.");
		}
	}
		...
		try {
			...
			ClassLoader patchClassLoader = new ClassLoader(classLoader) {
				...
				protected Class<?> findClass(String className)
					...
					if (clazz != null) {
						Util_LOGFile.getInstance().write(TAG + "\n" + "类以什么开头:" + clazz.getName());
						Log.d(TAG, "类以什么开头:" + clazz.getName());
					}
	...
	private void fixClass(Class<?> clazz, ClassLoader classLoader) {
		...
		for (Method method : methods) {
			...
			if (!isEmpty(clz) && !isEmpty(meth)) {
				Util_LOGFile.getInstance().write(TAG + "\n" + "fixClass clz类名是:" + clz.toString() + " meth名字为:" + meth
						.toString());
				Log.d(TAG, "fixClass clz类名是:" + clz.toString() + " meth名字为:" + meth
						.toString());
	...
	private void replaceMethod(ClassLoader classLoader, String clz,
		...
		try {
			...
			if (clazz == null) {// class not load
				...
				Util_LOGFile.getInstance().write(TAG + "\n" + "AndFix 先初始化clazz 成功 " + clazz);
				Log.d(TAG, "AndFix  初始化clazz成功 " + clazz);
			}
			if (clazz != null) {// initialize class OK
				...
				Util_LOGFile.getInstance().write(TAG + "\n" + "AndFix 替换clazz方法 成功 " + src);
				Log.d(TAG, "AndFix 替换clazz方法成功 " + src);
			}
		} catch (Exception e) {
			Util_LOGFile.getInstance().write(TAG + "\n" + "replaceMethod  " + e.toString());
